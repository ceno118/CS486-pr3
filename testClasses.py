...
class WeightedNumberPassedQuestion(Question):
    """Grade is the number of test cases passed * point per question."""

    def __init__(self, questionDict, display):
        super().__init__(questionDict, display)
        self.weight = int(questionDict['weight'])

    def execute(self, grades):
        grades.addPoints([f(grades) for _, f in self.testCases].count(True) * self.weight)
