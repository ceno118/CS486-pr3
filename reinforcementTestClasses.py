...
class Question6Test(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        # Schrum: had to encode to UTF to avoid error in Python 3+
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        self.maxPoints = (int(testDict['max_points']) if 'max_points' in testDict
                else sum([len(t) for t in [self.scoreThresholds, self.nonTimeoutThresholds, self.winsThresholds]]))
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            #     self.addMessage("%s %s (%s of %s points)" % (value, name, points, len(thresholds)))
                self.addMessage("%s %s (%s of %s points)" % (value, name, points, len(thresholds)*3))
            ...
            if minimum != None:
                ...
                for idx, threshold in enumerate(thresholds):
                    ...
                    self.addMessage("    >= %s:  %s points" % (threshold, (idx+1)*3))
